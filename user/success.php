...
try {
    ...
    $stripe_user_id = $session->customer ?? null; // Get Stripe customer ID
$payment_method = $payment_intent->payment_method_types[0] ?? null; // Get payment method type
    ...
    try {
        // Save payment
        savePayment($con, $payment_id, $payment_status, $user_id, $total_price, $stripe_user_id, $payment_method);
...
function savePayment($con, $payment_id, $payment_status, $user_id, $total_price, $stripe_user_id, $payment_method)
{
    $query = "
        INSERT INTO payments (payment_id, payment_status, user_id, total_price, stripe_user, payment_method, created_at) 
        VALUES ('$payment_id', '$payment_status', $user_id, $total_price, '$stripe_user_id', '$payment_method', NOW())";
...
function updateSizeAvailability($con, $produkt_id, $size, $quantity)
{
    ...
    if (mysqli_affected_rows($con) == 0) {
        throw new Exception("No rows updated. Verify that produkt_id: $produkt_id and size: '$size' exist in the database.");
    }
}
...
function displaySuccessPage($order_id, $total_price)
{
    ...
    <div class="container">
        ...
        <a href="my_orders.php" class="btn" style="margin-left: 10px;">View Order History</a>
    </div>
    <div class="footer">
