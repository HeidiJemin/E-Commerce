...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($_POST['action'])) {
        if ($_POST['action'] === 'register') {
            ...
            $query_insert = "INSERT INTO users (name, surname,foto, username, email, password, role_id, verified) VALUES ('$name', '$surname', '$username', '$email', '$passwordHashed', 1, 0)";
        ...
        elseif ($_POST['action'] == "login") {
            ...
            if (!password_verify($password, $passwordHashed)) {
                ...
                // Lock the account immediately if the failed attempts reach 7
if ($new_failed_attempts >= 7) {
    $lockout_time = date("Y-m-d H:i:s");
    $query_lockout = "UPDATE users SET failed_attempts = $new_failed_attempts, lockout_time = '$lockout_time' WHERE user_id = " . $row['user_id'];
    mysqli_query($con, $query_lockout);

    http_response_code(203);
    echo json_encode([
        "message" => "Your account is locked due to too many failed attempts. Please try again after 30 minutes."
    ]);
    exit;
}

// Otherwise, update the failed attempts without locking the account
$query_update_failed_attempts = "UPDATE users SET failed_attempts = $new_failed_attempts WHERE user_id = " . $row['user_id'];
mysqli_query($con, $query_update_failed_attempts);
